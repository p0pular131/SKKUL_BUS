...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def request_new_actor(model, spawn_point, rolename='scenario', autopilot=False,
        ...
        blueprint = CarlaDataProvider.create_blueprint('vehicle.mitsubishi.fusorosa', rolename, color, "truck", attribute_filter)
        random_location = False
        if random_location:
            actor = None
            while not actor:
                CarlaDataProvider.set_random_seed(234243)
                CarlaDataProvider.generate_spawn_points()
